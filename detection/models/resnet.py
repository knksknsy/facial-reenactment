...
class ResNet(nn.Module):
    def __init__(self, block, layers, len_feature=1000, zero_init_residual=False, groups=1, width_per_group=64, replace_stride_with_dilation=None, norm_layer=None):
    ...
    def _forward_impl(self, x):
        ...
        x = self.layer2(x)
        x = self.layer3(x)
        l4_output = self.layer4(x)
		

        x = self.avgpool(l4_output)
        x = torch.flatten(x, 1)
        x = self.fc(x)
