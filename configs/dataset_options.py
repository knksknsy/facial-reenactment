...
class DatasetOptions(Options):
    ...
    def _init_parser(self):
        ...
        self.parser.add_argument('--num_pairs', type=int, default=self.config['preprocessing']['num_pairs'],
                                            help='Number of training pairs (frames) to extract from a video.')
        self.parser.add_argument('--max_frames', type=int, default=self.config['preprocessing']['max_frames'],
                                            help='Number of max frames to extract from a video.')
        ...
        self.parser.add_argument('--padding', type=int, default=self.config['preprocessing']['padding'],
                                            help='Padding size')
        self.parser.add_argument('--padding_color', nargs='+', type=int, default=self.config['preprocessing']['padding_color'],
                                            help='Padding color')
        ...
        self.parser.add_argument('--prune_videos', action='store_false' if self.config['preprocessing']['prune_videos'] else 'store_true',
                                            help='Split large videos into chunks to fit video into RAM. Use if RAM < 16 GB')
